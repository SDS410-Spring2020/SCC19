...
# Unlist vector of 
titles_dates <- unlist(titles_dates)

# Date format to search for
date <- "[A-Z]{1}[a-z]{1,10} [0-9]{1,2}, [0-9]{4}"

# Extract date using above format from titles
dates <- lapply(titles_dates,
                function(titles_dates) {
                 str_extract(string = titles_dates,
                             pattern = date)
                }
 )
...
df_dates <- as.data.frame(cbind(dates, link))
